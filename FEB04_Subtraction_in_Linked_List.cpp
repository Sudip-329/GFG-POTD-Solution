...
public:
    ...
    pair<Node *, Node *> subtract(Node * head1, Node * head2) {
        ...
        while(head1 or head2){    //when both will be NULL then stop only
            int cur = (head1 ? head1 -> data : 0) - (head2 ? head2 -> data : 0) - carry;
            carry = cur < 0;  //(head2 ? head2 -> data : 0)  again checking if null then use value 0.
            
            if(carry){  //if carry is needed
                if((head1 and head1 -> next) or (head2 and head2 -> next)){  //checking in left any number left or not
                    ans.insert(10 + cur);
                    second.insert(0);
                }
                else{
